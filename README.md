...
Add Filter 的作用是增加一些字段到事件中, 也可以通过将 overwrite 设置为 true 修改已有的字段.
...
Convert Filter 的作用是将一个字段转换成另外一个类型, 目前支持的类型有 float/int/string/bool
...
可以将一个json字符串解析成一个json对象, 解析成功后, 会将解析出来的字段放到目标字段中.
...
目标字段, 如果不设置, 则将 Json Filter 生成的所有字段写入到根一层.
