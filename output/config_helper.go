...
// SafeDecodeConfig safely decodes output configuration using encoding/json
// This provides detailed error messages from the standard library
func SafeDecodeConfig(outputType string, config map[any]any, result any) {
	...
	if err != nil {
		panic(fmt.Sprintf("%s output: failed to marshal config to JSON: %v", outputType, err))
	}
	...
	if err := decoder.Decode(result); err != nil {
		panic(fmt.Sprintf("%s output configuration error: %v", outputType, err))
	}
}

// ValidateRequiredFields checks that required fields are present in the decoded config
func ValidateRequiredFields(outputType string, fields map[string]any) {
	for fieldName, fieldValue := range fields {
		if fieldValue == nil || (fmt.Sprintf("%v", fieldValue) == "") {
			panic(fmt.Sprintf("%s output: '%s' is required", outputType, fieldName))
		}
	}
}